...
# Collect all files in target folder
    $files = Get-ChildItem -Path "C:\Users\zthie\Desktop\file_integrity_monitor\Files"
    
# For File, calculate the hash, and write to baseline.txt
    foreach ($f in $files) {
        $hash = Calculate-File-hash $f.FullName
        "$($hash.Path)|$($hash.Hash)" | Out-File -FilePath "C:\Users\zthie\Desktop\file_integrity_monitor\Files\baseline.txt" -Append
    }
...
# Load hashes from baseline.txt and store in a dictionary
    $filesPandH = Get-Content -Path "C:\Users\zthie\Desktop\file_integrity_monitor\Files\baseline.txt"
